...
impl Keyboard {
    ...
    fn key_decoded(&mut self) -> bool {
        ...
        if (self.code & BREAK_BIT) != 0 {
            ...
            match self.code {
                ...
                56 => {
                    if self.prefix == PREFIX1 { self.gather.set_alt_right(false); } else { self.gather.set_alt_left(false); }
                }
                29 => {
                    if self.prefix == PREFIX1 { self.gather.set_ctrl_right(false); } else { self.gather.set_ctrl_left(false); }
                }
                _ => { // All other keys
                }
            }
        ...
        match self.code {
            ...
            56 => {
                if self.prefix == PREFIX1 { self.gather.set_alt_right(true); } else { self.gather.set_alt_left(true); }
            }
            29 => {
                if self.prefix == PREFIX1 { self.gather.set_ctrl_right(true); } else { self.gather.set_ctrl_left(true); }
            }
            58 => {
    ...
    pub fn key_hit(&mut self) -> Key {
        ...
        // loop {
        //     unsafe {
        //         let mut readbit = self.control_port.inb();
        //         if readbit & KBD_OUTB != 0 {
        //             self.code = self.data_port.inb();
        //             if readbit & KBD_AUXB != 1 {
        //                 return if self.key_decoded() {
        //                     self.gather.clone()
        //                 } else {
        //                     invalid
        //                 };
         ...
         *                  werden, bis ein Zeichen eingegeben wurde.                 *
         *                                                                           *
         ...
         *                  sowie gegebenenfalls ein ASCII-Code ermittelt werden      *
        ...
        loop {
            unsafe {
                ...
                self.code = self.data_port.inb();
                // if self.control_port.inb() & KBD_AUXB != 1 {
                    return if self.key_decoded() {
                        self.gather.clone()
                    } else {
                        invalid
